...
public class PriceProductsService {
	...
	protected Logger logger = Logger.getLogger(PriceProductsService.class
			.getName());
	...
	public List<Account> byOwnerContains(String name) {
		...
		try {
			accounts = restTemplate.getForObject(serviceUrl
					+ "/products/owner/{name}", Account[].class, name);
	...
	public Account getByNumber(String accountNumber) {
		Account account = restTemplate.getForObject(serviceUrl
				+ "/products/{number}", Account.class, accountNumber);

		if (account == null)
			throw new ProductstNotFoundException(accountNumber);
		else
			return account;
	}
}
